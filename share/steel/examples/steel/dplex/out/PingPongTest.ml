...
let client (c : Obj.t Steel_Channel_Duplex.chan) : unit=
  Steel_Channel_Duplex.send pingpong c pingpong (Obj.magic (Prims.of_int 17));
  (let y =
     Steel_Channel_Duplex.recv pingpong
       (Steel_Channel_Protocol.step pingpong (Obj.magic (Prims.of_int 17))) c in
   ())
    ...
    Steel_Primitive_ForkJoin.fork () () () () client_server
      (fun uu___1 uu___ ->
         (fun t ->
            ...
            fun uu___ -> Obj.magic (many (n - Prims.int_one) (t :: threads)))
           uu___1 uu___)
