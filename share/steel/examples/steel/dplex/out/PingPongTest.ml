...
let (client : Obj.t Steel_Channel_Duplex.chan -> unit) =
  fun c ->
    Steel_Channel_Duplex.send pingpong c pingpong
      (Obj.magic (Prims.of_int (17)));
    (let y =
       Steel_Channel_Duplex.recv pingpong
         (Steel_Channel_Protocol.step pingpong
            (Obj.magic (Prims.of_int (17)))) c in
     ())
let (server : Obj.t Steel_Channel_Duplex.chan -> unit) =
  fun c ->
    let y =
      Steel_Channel_Duplex.recv pingpong
        (Steel_Channel_Protocol.dual pingpong) c in
...
let (client_server : unit -> unit) =
  fun uu___ ->
    ...
    ()
let rec (join_all : unit Steel_Primitive_ForkJoin.thread Prims.list -> unit)
  =
  fun threads ->
...
let rec (many :
  Prims.nat -> unit Steel_Primitive_ForkJoin.thread Prims.list -> unit) =
  fun n ->
    fun threads ->
      if n = Prims.int_zero
          ...
          (fun uu___2 ->
             fun uu___1 ->
               (fun t ->
                  ...
                  fun uu___1 ->
                    Obj.magic (many (n - Prims.int_one) (t :: threads)))
                 uu___2 uu___1)
