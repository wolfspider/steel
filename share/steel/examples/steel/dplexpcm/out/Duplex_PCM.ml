...
let return : 'a. 'a -> 'a nl_protocol =
 fun uu___ ->
  (fun x -> Obj.magic (Steel_Channel_Protocol.Return ((), Obj.magic x))) uu___
    ...
    Obj.magic
      (Steel_Channel_Protocol.Msg
         ( Steel_Channel_Protocol.Send,
           (),
           (),
           fun uu___1 -> (Obj.magic return) uu___1 )))
    uu___
...
let rec bind : 'a 'b. 'a nl_protocol -> ('a -> 'b nl_protocol) -> 'b nl_protocol
    =
 fun uu___1 ->
  fun uu___ ->
   (fun p ->
...
let (uu___is_V : dprot -> Obj.t t -> Prims.bool) =
 fun p -> fun projectee -> match projectee with V _0 -> true | uu___ -> false
...
let (uu___is_A_W : dprot -> Obj.t t -> Prims.bool) =
 fun p ->
  fun projectee -> match projectee with A_W (q, _1) -> true | uu___ -> false
...
let (uu___is_A_R : dprot -> Obj.t t -> Prims.bool) =
 fun p ->
  fun projectee -> match projectee with A_R (q, _1) -> true | uu___ -> false
...
let (uu___is_B_R : dprot -> Obj.t t -> Prims.bool) =
 fun p ->
  fun projectee -> match projectee with B_R (q, _1) -> true | uu___ -> false
...
let (uu___is_B_W : dprot -> Obj.t t -> Prims.bool) =
 fun p ->
  fun projectee -> match projectee with B_W (q, _1) -> true | uu___ -> false
...
let (uu___is_A_Fin : dprot -> Obj.t t -> Prims.bool) =
 fun p ->
  fun projectee -> match projectee with A_Fin (q, _1) -> true | uu___ -> false
...
let (uu___is_B_Fin : dprot -> Obj.t t -> Prims.bool) =
 fun p ->
  fun projectee -> match projectee with B_Fin (q, _1) -> true | uu___ -> false
...
let (uu___is_Nil : dprot -> Obj.t t -> Prims.bool) =
 fun p -> fun projectee -> match projectee with Nil -> true | uu___ -> false
  ...
  fun q ->
   fun s ->
    ...
    | Steel_Channel_Protocol.Message (uu___, uu___1, uu___2, t1) ->
        Prims.int_one
        + trace_length (Steel_Channel_Protocol.step uu___ uu___1) uu___2 t1
 ...
 fun p ->
  fun s0 ->
   fun s1 ->
    ...
    | Nil, a -> a
    | A_Fin (q, s), uu___ ->
        V { Steel_Channel_Protocol.to1 = q; Steel_Channel_Protocol.tr = s }
    | uu___, A_Fin (q, s) ->
        V { Steel_Channel_Protocol.to1 = q; Steel_Channel_Protocol.tr = s }
    | B_Fin (q, s), uu___ ->
        V { Steel_Channel_Protocol.to1 = q; Steel_Channel_Protocol.tr = s }
    ...
    | A_R (q', s'), B_W (q, s) ->
        V { Steel_Channel_Protocol.to1 = q; Steel_Channel_Protocol.tr = s }
    | A_R (q, s), B_R (q', s') ->
        if trace_length p q s >= trace_length p q' s' then
          V { Steel_Channel_Protocol.to1 = q; Steel_Channel_Protocol.tr = s }
        else
          V { Steel_Channel_Protocol.to1 = q'; Steel_Channel_Protocol.tr = s' }
    | B_R (q', s'), A_R (q, s) ->
...
let (p' : dprot -> Obj.t t FStar_PCM.pcm') =
 fun p ->
  { FStar_PCM.composable = (); FStar_PCM.op = compose p; FStar_PCM.one = Nil }
...
let (ep_a :
 ...
 fun p ->
  fun next1 ->
   fun tr ->
    if is_send next1 then A_W (next1, tr)
    else if is_recv next1 then A_R (next1, tr)
    else A_Fin (next1, tr)
...
let (select_refine' : dprot -> Obj.t chan -> unit -> unit -> Obj.t t) =
 fun p ->
  fun r -> fun x -> fun f -> Steel_PCMReference.select_refine (pcm p) r () ()
     ...
     fun tr' ->
      ...
      | Steel_Channel_Protocol.Message (uu___, x, to2, tail) -> (
          ...
          | Steel_Channel_Protocol.Message (uu___2, x', to'1, tail') ->
              next_message_aux
                (Steel_Channel_Protocol.step uu___ x)
                to2 to'1 tail tail')
    ...
    fun tr' ->
     let x =
       next_message p q
         ...
         tr tr'.Steel_Channel_Protocol.tr
     in
     let q' = Steel_Channel_Protocol.step q x in
     let tr'1 = Steel_Channel_Protocol.extend p q tr x in
     if is_send q' then A_W (q', tr'1)
     else if is_recv q' then A_R (q', tr'1)
     else A_Fin (q', tr'1)
  ...
  fun q ->
   fun tr ->
    fun tr' ->
     ...
     let q' = Steel_Channel_Protocol.step q x in
     let tr'1 = Steel_Channel_Protocol.extend p q tr x in
     if is_send q' then B_R (q', tr'1)
     else if is_recv q' then B_W (q', tr'1)
     else B_Fin (q', tr'1)
...
let get_b_r p c next1 tr =
  let _ =
    Steel_Effect_Atomic.as_atomic_action () () () (fun h ->
        let a =
          match (c : Obj.t chan) with
          | r -> (
              match r with
              | Steel_Heap.Addr a -> a
              | Steel_Heap.Null -> failwith "Null chan")
        in
...
let pp_t (v : Obj.t t) =
  ...
  | V _ -> "V"
  | A_W (q, _) ->
      "A_W(" ^ pp_tag (Steel_Channel_Protocol.__proj__Msg__item___0 q) ^ ")"
  | A_R (q, _) ->
      "A_R(" ^ pp_tag (Steel_Channel_Protocol.__proj__Msg__item___0 q) ^ ")"
  | B_R (q, _) ->
      "B_R(" ^ pp_tag (Steel_Channel_Protocol.__proj__Msg__item___0 q) ^ ")"
  | B_W (q, _) ->
      "B_W(" ^ pp_tag (Steel_Channel_Protocol.__proj__Msg__item___0 q) ^ ")"
  | A_Fin (q, _) ->
      "A_Fin("
      ^ (if Steel_Channel_Protocol.uu___is_Return q then "Return" else "?")
      ^ ")"
  | B_Fin (q, _) ->
      "B_Fin("
      ^ (if Steel_Channel_Protocol.uu___is_Return q then "Return" else "?")
...
let upd_gen_action (p : dprot) (r : Obj.t chan) (_x : Obj.t t) (_y : Obj.t t)
    (f : (Obj.t t, Obj.t, Obj.t, Obj.t) FStar_PCM.frame_preserving_upd) : unit =
...
let write_a_f_aux p next1 tr x :
    (Obj.t t, _, _, _) FStar_PCM.frame_preserving_upd =
 fun old ->
  match old with
  | V tr0 ->
...
let write_b_f_aux p next1 tr x :
    (Obj.t t, _, _, _) FStar_PCM.frame_preserving_upd =
 fun old ->
  match old with
  | V tr0 ->
...
let write_a p r next1 tr x =
  upd_gen_action p r
    (A_W (next1, tr))
    (if is_send (Steel_Channel_Protocol.step next1 x) then
       A_W
         ( Steel_Channel_Protocol.step next1 x,
           Steel_Channel_Protocol.extend p next1 tr x )
     else if is_recv (Steel_Channel_Protocol.step next1 x) then
       A_R
         ( Steel_Channel_Protocol.step next1 x,
           Steel_Channel_Protocol.extend p next1 tr x )
     else
...
let show_nat (n : Prims.nat) : string = string_of_int (Obj.magic n : int)
...
let rec recv_a p c next1 tr =
  ...
  if l_local >= l_remote then (
    pause ();
    recv_a p c next1 tr)
  else next_message p next1 to' tr tr'.Steel_Channel_Protocol.tr
  ...
  fun name ->
   fun c ->
    fun next1 ->
     fun x ->
      fun t1 ->
       if name = A then send_a p c next1 x t1 else send_b p c next1 x t1
         ...
         let w' =
           Prims.Mkdtuple2
             ( Steel_Channel_Protocol.step (Obj.magic next1) x,
               Steel_Channel_Protocol.extend p (Obj.magic next1) tr x )
         in
...
let (channel_send' :
 ...
 fun name p next1 c x ->
  ...
  pack_trace_ref p name c
    (Prims.Mkdtuple2 (next1, tr))
    true (Obj.magic next1) tr x

let (ch_as_channel : party -> ch -> dprot -> unit) =
 fun _party -> fun _c -> fun _prot -> ()

let (channel_send : party -> Obj.t send_next_dprot_t -> ch -> Obj.t -> unit) =
 fun name ->
  fun next1 ->
   fun c ->
    fun x ->
  ...
  fun p ->
   fun next1 ->
    fun c ->
     ...
     pack_trace_ref p name c
       (Prims.Mkdtuple2 (next1, tr))
       false (Obj.magic next1) tr x;
     x

let (channel_recv : party -> Obj.t recv_next_dprot_t -> ch -> Obj.t) =
 fun name ->
  fun next1 ->
   fun c ->
    ch_as_channel name c next1;
