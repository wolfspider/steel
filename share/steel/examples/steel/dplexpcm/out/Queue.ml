open Prims
let pure_upd_next :
  'a . 'a Queue_Def.cell -> 'a Queue_Def.t -> 'a Queue_Def.cell =
  fun c ->
    fun next ->
      { Queue_Def.data = (c.Queue_Def.data); Queue_Def.next = next }
...
let rec next_last :
  ...
  fun pstart ->
    fun l ->
      match l with
      | [] -> pstart
      | (uu___, c)::q -> next_last c.Queue_Def.next q
let get_data :
...
let new_queue : 'a . 'a -> 'a Queue_Def.t =
  fun v ->
    let c =
      { Queue_Def.data = v; Queue_Def.next = (NullCompat.S.null ()) } in
  ...
  fun u ->
    fun hd ->
      fun tl ->
        fun v ->
          fun last ->
            upd_next () () tl last; Steel_Effect_Atomic.return () () ()
...
let dequeue :
  'a .
    unit ->
      unit -> 'a Queue_Def.t -> 'a Queue_Def.t FStar_Pervasives_Native.option
  =
  fun _u _tl hd ->
    let p = read_next () () hd in
    if NullCompat.S.is_null p
    then Steel_Effect_Atomic.return () () FStar_Pervasives_Native.None
