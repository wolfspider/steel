  ...
  fun uu___ ->
    (fun p ->
       ...
       | Msg (tag1, b, a1, k) ->
           Obj.magic
             (Obj.repr
                (let k1 x = dual (k x) in Msg ((flip_tag tag1), (), (), k1)))
       | DoWhile (p1, a1, k) ->
           Obj.magic (Obj.repr (DoWhile ((dual p1), (), (dual k))))) uu___
               ...
               Obj.magic
                 (Obj.repr
                    (DoWhile
                       (w, (),
                         (Obj.magic
                            (bind k (fun uu___1 -> (Obj.magic q) uu___1)))))))
        uu___1 uu___
let op_let_Hat :
  'uuuuu 'uuuuu1 .
    unit ->
      'uuuuu protocol -> ('uuuuu -> 'uuuuu1 protocol) -> 'uuuuu1 protocol
  = fun uu___ -> bind
    ...
    (fun uu___ ->
       Obj.magic
         (Msg
            (Send, (), (),
              (fun uu___1 ->
                 (fun x -> let x = Obj.magic x in Obj.magic (return x))
                   uu___1)))) uu___
    ...
    (fun p ->
       ...
       | DoWhile (p1, uu___, k) ->
           Obj.magic
             (Obj.repr
                (bind (hnf p1)
                   (fun b -> if b then DoWhile (p1, (), k) else k)))
       | uu___ -> Obj.magic (Obj.repr p)) uu___
type ('a, 'p) next_msg_t = Obj.t
let step : 'a . 'a protocol -> Obj.t -> 'a protocol =
  fun uu___1 ->
    fun uu___ ->
      (fun p -> fun x -> Obj.magic (__proj__Msg__item__k (hnf p) x)) uu___1
        uu___
...
let (uu___is_Waiting :
  ...
  fun from ->
    fun to1 ->
      fun projectee ->
        match projectee with | Waiting p -> true | uu___ -> false
let (__proj__Waiting__item__p :
  unit protocol -> unit protocol -> (Obj.t, Obj.t) trace -> unit protocol) =
  fun from ->
    fun to1 -> fun projectee -> match projectee with | Waiting p -> p
let (uu___is_Message :
  unit protocol -> unit protocol -> (Obj.t, Obj.t) trace -> Prims.bool) =
  fun from ->
    fun to1 ->
...
let (__proj__Message__item__from :
  ...
  fun from ->
    fun to1 ->
      fun projectee ->
        match projectee with | Message (from1, x, to2, _3) -> from1
let (__proj__Message__item__x :
  unit protocol -> unit protocol -> (Obj.t, Obj.t) trace -> Obj.t) =
  fun from ->
    fun to1 ->
      fun projectee ->
        match projectee with | Message (from1, x, to2, _3) -> x
let (__proj__Message__item__to :
  unit protocol -> unit protocol -> (Obj.t, Obj.t) trace -> unit protocol) =
  fun from ->
    fun to1 ->
      fun projectee ->
        match projectee with | Message (from1, x, to2, _3) -> to2
let (__proj__Message__item___3 :
  unit protocol ->
    unit protocol -> (Obj.t, Obj.t) trace -> (Obj.t, Obj.t) trace)
  =
  fun from ->
    fun to1 ->
      fun projectee ->
        match projectee with | Message (from1, x, to2, _3) -> _3
let rec (extend :
  unit protocol ->
    unit protocol -> (Obj.t, Obj.t) trace -> Obj.t -> (Obj.t, Obj.t) trace)
  =
  fun from ->
    fun to1 ->
      fun t ->
        fun m ->
          match t with
          | Waiting uu___ ->
              Message (to1, m, (step to1 m), (Waiting (step to1 m)))
          | Message (_from, x, _to, tail) ->
              Message
                (_from, x, (step _to m), (extend (step _from x) _to tail m))
