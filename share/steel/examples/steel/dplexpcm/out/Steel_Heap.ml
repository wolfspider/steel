open Prims
type cell =
  | Ref of unit * Obj.t FStar_PCM.pcm * unit * Obj.t 
...
type addr = Prims.nat
type heap =
  (addr, cell FStar_Pervasives_Native.option)
...
let (update_addr' :
  heap -> addr -> cell FStar_Pervasives_Native.option -> heap) =
  fun m -> fun a -> fun c -> fun x -> if a = x then c else m x
let (update_addr : heap -> addr -> cell -> heap) =
  fun m ->
    fun a -> fun c -> update_addr' m a (FStar_Pervasives_Native.Some c)
...
let string_of_pos (p : Prims.pos) : string =
  try Z.to_string (Obj.magic p : Z.t) with _ -> string_of_int (Obj.magic p : int)
...
let sel_action :
  'a .
    'a FStar_PCM.pcm -> ('a, Obj.t) ref -> unit -> (Obj.t, 'a, Obj.t) action
  =
  fun pcm ->
    fun r -> fun v0 -> let f m0 = Prims.Mkdtuple2 ((sel pcm r m0), m0) in f
let sel_action' :
  'a . 'a FStar_PCM.pcm -> ('a, Obj.t) ref -> unit -> Obj.t full_hheap -> 'a
...
let (refined_pre_action_as_action :
  ...
  = fun fp0 -> fun a -> fun fp1 -> fun f -> let g m = f m in g
let select_refine_pre :
  'a .
    'a FStar_PCM.pcm ->
      ('a, Obj.t) ref ->
        unit -> unit -> (Obj.t, 'a, Obj.t) refined_pre_action
  =
  fun p ->
    fun r ->
      fun x ->
        fun f -> fun h0 -> let v = sel_v p r () h0 in Prims.Mkdtuple2 (v, h0)
let select_refine :
  'a .
    'a FStar_PCM.pcm ->
      ('a, Obj.t) ref -> unit -> unit -> (Obj.t, 'a, Obj.t) action
  =
  fun uu___3 ->
    fun uu___2 ->
      fun uu___1 ->
                 ...
                 fun f ->
                   Obj.magic
                     (refined_pre_action_as_action () () ()
                        (Obj.magic (select_refine_pre p r () ())))) uu___3
            uu___2 uu___1 uu___
let (update_addr_full_heap : full_heap -> addr -> cell -> full_heap) =
  fun h -> fun a -> fun c -> let h' = update_addr h a c in h'
          ...
          fun f ->
            fun h ->
              ...
              | Ref (a1, p1, frac, old_v) ->
                  ...
                  let h' =
                    update_addr_full_heap h (__proj__Addr__item___0 r) cell1 in
                  Prims.Mkdtuple2 ((), h')
let upd_gen_action :
  'a .
    'a FStar_PCM.pcm ->
                     ...
                     fun f ->
                       let refined h =
                         ...
                         | Prims.Mkdtuple2 (u, h1) ->
                             let h11 = h1 in Prims.Mkdtuple2 ((), h11) in
                       Obj.magic
                         (refined_pre_action_as_action () () ()
                            (Obj.magic refined))) uu___4 uu___3 uu___2 uu___1
              uu___
let upd_action :
  'a .
    'a FStar_PCM.pcm ->
      ('a, Obj.t) ref -> unit -> 'a -> (Obj.t, unit, Obj.t) action
  =
  fun pcm ->
    fun r ->
      fun v0 ->
...
let free_action :
  'a .
    'a FStar_PCM.pcm ->
      ('a, Obj.t) ref -> unit -> (Obj.t, unit, Obj.t) action
  =
  fun pcm ->
    fun r ->
      fun v0 ->
        let one = (pcm.FStar_PCM.p).FStar_PCM.one in
        let f v = one in upd_gen_action pcm r () () f
let split_action :
  'a .
    'a FStar_PCM.pcm ->
      ('a, Obj.t) ref -> unit -> unit -> (Obj.t, unit, Obj.t) action
  =
  fun uu___3 ->
    fun uu___2 ->
      fun uu___1 ->
             ...
             fun r ->
               fun v0 ->
                 fun v1 ->
                   ...
                   Obj.magic
                     (refined_pre_action_as_action () () () (Obj.magic g)))
...
let frame :
  'a .
    unit ->
      unit -> unit -> (Obj.t, 'a, Obj.t) action -> (Obj.t, 'a, Obj.t) action
  =
  fun uu___3 ->
    fun uu___2 ->
      fun uu___1 ->
               ...
               fun frame1 ->
                 fun f ->
                   let g h0 =
                     let uu___ = f h0 in
                     match uu___ with
                     | Prims.Mkdtuple2 (x, h1) -> Prims.Mkdtuple2 (x, h1) in
                   Obj.magic
                     (refined_pre_action_as_action () () () (Obj.magic g)))
            uu___3 uu___2 uu___1 uu___
...
let lift_h_exists : 'a . unit -> (Obj.t, unit, Obj.t) action =
  fun uu___ ->
    (fun p ->
       let g h = Prims.Mkdtuple2 ((), h) in
       Obj.magic (refined_pre_action_as_action () () () (Obj.magic g))) uu___
let elim_pure : 'p . unit -> (Obj.t, unit, Obj.t) action =
  fun uu___ ->
    (fun uu___ ->
       let f h = Prims.Mkdtuple2 ((), h) in
       Obj.magic (refined_pre_action_as_action () () () (Obj.magic f))) uu___
