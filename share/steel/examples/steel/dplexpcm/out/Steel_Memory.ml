...
let (join : mem -> mem -> mem) =
  fun m0 ->
    fun m1 ->
      {
        ctr = (m0.ctr);
        heap = (Steel_Heap.join m0.heap m1.heap);
        locks = (m0.locks)
      }
type slprop = unit
...
let null : 'a . 'a FStar_PCM.pcm -> ('a, Obj.t) ref =
  fun pcm -> core_ref_null
...
let (extend_lock_store :
  ...
  fun e ->
    fun l ->
      fun p ->
        Prims.Mkdtuple2
          ((FStar_List_Tot_Base.length l), ((Invariant ()) :: l))
...
type ('fp, 'a, 'fpu) tot_action_nf =
  (Obj.t, 'fp, 'a, 'fpu) tot_action_nf_except
let (hheap_of_hmem :
  unit ->
    unit -> (Obj.t, Obj.t) hmem_with_inv_except -> Obj.t Steel_Heap.hheap)
  = fun fp -> fun e -> fun m -> let h = heap_of_mem m in h
let (hmem_of_hheap :
  unit ->
    unit ->
      unit ->
        (Obj.t, Obj.t) hmem_with_inv_except ->
          Obj.t Steel_Heap.full_hheap -> (Obj.t, Obj.t) hmem_with_inv_except)
  =
  fun e ->
    fun fp0 ->
      fun fp1 ->
        fun m ->
          fun h ->
            let m1 = { ctr = (m.ctr); heap = h; locks = (m.locks) } in m1
        ...
        fun e ->
          fun f ->
            let g m =
              ...
              match uu___ with
              | Prims.Mkdtuple2 (x, h') ->
                  Prims.Mkdtuple2 (x, (hmem_of_hheap () () () m h')) in
            g

let lift_tot_action_nf :
  'a. unit -> unit -> unit ->
  (Obj.t, Obj.t, 'a, Obj.t) tot_action_nf_except -> 'a
=
  fun _e _fp _fp' f ->
    let (h, ctr) = FStar_NMSTTotal.get () in
    let m0 : full_mem = { ctr = ctr; heap = h; locks = [] } in
...
let lift_tot_action :
  'a .
    unit ->
      unit ->
        unit -> (Obj.t, Obj.t, 'a, Obj.t) tot_action_nf_except -> unit -> 'a
  =
  fun e ->
    fun fp ->
      fun fp' ->
        fun f ->
          fun frame ->
            let m0 = FStar_NMSTTotal.get () in lift_tot_action_nf () () () f
...
let sel_action :
  'a .
    'a FStar_PCM.pcm ->
      unit ->
        ('a, Obj.t) ref -> unit -> ('a, Obj.t, Obj.t, Obj.t) action_except
  =
  fun pcm ->
    fun e ->
      fun r ->
...
let upd_action :
  'a .
    'a FStar_PCM.pcm ->
      unit ->
        ('a, Obj.t) ref ->
          unit -> 'a -> (unit, Obj.t, Obj.t, Obj.t) action_except
  =
  fun pcm ->
    fun e ->
      fun r ->
...
let free_action :
  'a .
    'a FStar_PCM.pcm ->
      unit ->
        ('a, Obj.t) ref -> unit -> (unit, Obj.t, Obj.t, Obj.t) action_except
  =
  fun pcm ->
    fun e ->
      fun r ->
...
let split_action :
  'a .
    'a FStar_PCM.pcm ->
      unit ->
        ('a, Obj.t) ref ->
          unit -> unit -> (unit, Obj.t, Obj.t, Obj.t) action_except
  =
  fun pcm ->
    fun e ->
      fun r ->
...
let (inc_ctr :
  ...
  fun p ->
    fun e ->
      fun m ->
        let m' =
          { ctr = (m.ctr + Prims.int_one); heap = (m.heap); locks = (m.locks)
          } in
        let m'1 = m' in m'1
      ...
      unit ->
        unit ->
          (Obj.t, Obj.t, Obj.t, Obj.t) refined_pre_action ->
            (Obj.t, Obj.t, Obj.t, Obj.t) tot_action_nf_except)
  = fun fp0 -> fun a -> fun fp1 -> fun e -> fun f -> let g m = f m in g
let alloc_action :
  'a .
    'a FStar_PCM.pcm ->
      unit -> 'a -> (('a, Obj.t) ref, Obj.t, Obj.t, Obj.t) action_except
  =
  fun pcm ->
    fun e ->
      fun x ->
...
let select_refine :
  'a .
    'a FStar_PCM.pcm ->
      unit ->
        ('a, Obj.t) ref ->
          unit -> unit -> ('a, Obj.t, Obj.t, Obj.t) action_except
  =
  fun p ->
    fun e ->
      fun r ->
...
let witness :
  ...
  =
  fun _pcm _e _r _fact _v _u ->
    fun _frame -> FStar_NMSTTotal.witness ()
...
type 'p inv =
  (unit, Obj.t witnessed_name_is_ok,
...
let (fresh_invariant :
  unit ->
    unit -> pre_inv Prims.list -> (unit, Obj.t, Obj.t, Obj.t) action_except)
  =
  fun _e ->
  fun _p ->
  fun ctx ->
            ...
            fun frame1 ->
              fun f ->
                fun frame0 ->
                  ...
                  let x = f () in let m1 = FStar_NMSTTotal.get () in x
let (change_slprop :
  ...
  fun opened_invariants ->
    fun p ->
      lift_tot_action () () ()
        (Obj.magic
           (lift_heap_action () () () ()
              (Obj.magic (Steel_Heap.elim_pure ()))))
