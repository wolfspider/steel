...
let read : 'a. 'a FStar_PCM.pcm -> ('a, Obj.t) Steel_Memory.ref -> unit -> 'a =
 fun pcm ->
  fun r ->
   fun v0 ->
    let v =
      Steel_Effect.as_action () () (Steel_Memory.sel_action pcm () r ())
    in
    v
...
let write :
 ...
 fun pcm ->
  fun r ->
   fun v0 ->
    fun v1 ->
     Steel_Effect.as_action () () (Steel_Memory.upd_action pcm () r () v1)

let alloc' : 'a. 'a FStar_PCM.pcm -> 'a -> ('a, Obj.t) Steel_Memory.ref =
 fun pcm ->
  fun x -> Steel_Effect.as_action () () (Steel_Memory.alloc_action pcm () x)

(* out/Steel_PCMReference.ml *)
let alloc (pcm : 'a FStar_PCM.pcm) (x : 'a) : ('a, Obj.t) Steel_Memory.ref =
  (* alloc_action already does the NMST get/put internally via lift_tot_action *)
  (Steel_Memory.alloc_action pcm () x) ()

let free : 'a. 'a FStar_PCM.pcm -> ('a, Obj.t) Steel_Memory.ref -> unit -> unit
    =
 fun p ->
  fun r ->
   fun x -> Steel_Effect.as_action () () (Steel_Memory.free_action p () r ())
   ...
   fun pcm ->
    fun r ->
     fun fact ->
      fun v ->
       fun _u2 ->
        Steel_Effect_Atomic.as_atomic_unobservable_action () () () (fun h ->
...
let select_refine (p : 'a FStar_PCM.pcm) (r : ('a, Obj.t) Steel_Memory.ref)
    (_x : unit) (_f : unit) : 'a =
...
(* out/Steel_PCMReference.ml *)
let upd_gen (pcm : 'a FStar_PCM.pcm) (r : ('a, Obj.t) Steel_Memory.ref)
    (_x : unit) (_y : unit)
    (f : ('a, Obj.t, Obj.t, Obj.t) FStar_PCM.frame_preserving_upd) : unit =
