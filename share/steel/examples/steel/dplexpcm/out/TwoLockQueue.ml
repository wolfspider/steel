...
type 'a t = { head : 'a q_ptr; tail : 'a q_ptr; inv : unit }
...
let enqueue : 'a. 'a t -> 'a -> unit =
 fun hdl ->
  fun x ->
   ...
   let cell = { Queue_Def.data = x; Queue_Def.next = NullCompat.S.null () } in
  ...
  fun hdl ->
   fun hd ->
    fun uu___ ->
     ...
     match o with
     | FStar_Pervasives_Native.None -> Steel_Effect_Atomic.return () () o
     | FStar_Pervasives_Native.Some p -> Steel_Effect_Atomic.return () () o
